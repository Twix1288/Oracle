...
-- Permissive policies (align with project's current approach)
create policy "Allow public insert access to mentor_profiles"
  on public.mentor_profiles for insert with check (true);
create policy "Allow public read access to mentor_profiles"
  on public.mentor_profiles for select using (true);
create policy "Allow public update access to mentor_profiles"
...
create policy "Allow public insert access to builder_onboarding"
  on public.builder_onboarding for insert with check (true);
create policy "Allow public read access to builder_onboarding"
  on public.builder_onboarding for select using (true);
create policy "Allow public update access to builder_onboarding"
  on public.builder_onboarding for update using (true);

-- Update timestamps triggers (drop if exists then create)
...
drop trigger if exists update_builder_onboarding_updated_at on public.builder_onboarding;
create trigger update_builder_onboarding_updated_at
...
-- Realtime: ensure full row data and publication
