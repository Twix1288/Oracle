...
CREATE INDEX idx_oracle_logs_vectorized ON oracle_logs(vectorized) WHERE vectorized = true;

-- Create vector similarity search index if pgvector is available
-- This will enable fast similarity searches across Oracle conversations
CREATE INDEX IF NOT EXISTS idx_oracle_logs_embedding 
ON oracle_logs USING ivfflat (embedding vector_cosine_ops)
WITH (lists = 100);
...
ALTER TABLE public.teams ADD COLUMN IF NOT EXISTS ai_summary text; -- AI-generated team summary for Oracle context

-- Function to update Oracle interaction stats
...
-- Updated trigger for profiles to update timestamp
